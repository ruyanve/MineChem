...
public class EasyPacketHandler {
	 ...
	 */
	@NotNull
	public static EasyPacketHandler registerEasyPacket(@NotNull Class<? extends EasyPacket> clazz, EasyPacketDispatcher dispatcher) {
	...
	private static void collectFields(@NotNull EasyPacketHandler handler, @NotNull Class<?> clazz) {
	...
	private static void registerDataField(@NotNull EasyPacketHandler handler, @NotNull Field field, EasyPacketData dataAnnotation) {
	...
	public void sendToServer(@NotNull EasyPacket packet) {
		byte[] data = write(packet);
		dispatcher.sendDataToServer(data, packet.isChunkDataPacket());
	}

	public void sendToAllPlayers(@NotNull EasyPacket packet) {
		byte[] data = write(packet);
		dispatcher.sendDataToAllPlayers(data, packet.isChunkDataPacket());
	}

	public void sendToAllPlayersInDimension(@NotNull EasyPacket packet, int dimensionID) {
		byte[] data = write(packet);
		dispatcher.sendDataToAllPlayersInDimension(data, packet.isChunkDataPacket(), dimensionID);
	}

	public void sendToAllPlayersAround(@NotNull EasyPacket packet, double x, double y, double z, double range, int dimensionID) {
		byte[] data = write(packet);
		dispatcher.sendDataToAllPlayersAround(data, packet.isChunkDataPacket(), x, y, z, range, dimensionID);
	}

	public void sendToPlayer(@NotNull EasyPacket packet, Player player) {
		byte[] data = write(packet);
