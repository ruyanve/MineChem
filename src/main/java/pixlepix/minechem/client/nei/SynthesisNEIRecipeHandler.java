...
public class SynthesisNEIRecipeHandler extends TemplateRecipeHandler {
	...
	private static final String MINECHEM_SYNTHESIS_RECIPES_ID =
			"minechem.synthesis";
	...
	public void loadUsageRecipes(ItemStack ingredient) {
		...
		// use it.
		if (!Util.isStackAChemical(ingredient)) {
			return;
		}
	 ...
	 */
	public void registerSynthesisRecipe(@Nullable SynthesisRecipe sr) {
		if (sr == null) {
			return;
		}
	...
	public class CachedSynthesisRecipe extends TemplateRecipeHandler.CachedRecipe {
		...
		public CachedSynthesisRecipe(SynthesisRecipe sr) {
				...
				for (Chemical c : inputChemicals) {
					if (c != null) {
						ItemStack inputItem = MinechemHelper.chemicalToItemStack(
								c, c.amount);
						inputs.add(new PositionedStack(inputItem,
								INPUT_X_OFS + xSlot * INPUT_X_SCALE,
								INPUT_Y_OFS + ySlot * INPUT_Y_SCALE));
					}
					xSlot++;
					if (xSlot > 2) {
						xSlot = 0;
			...
			output = new PositionedStack(sr.getOutput(),
					OUTPUT_X_OFS, OUTPUT_Y_OFS);
