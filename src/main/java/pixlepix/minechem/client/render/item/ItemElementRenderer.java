...
public class ItemElementRenderer implements IItemRenderer {
	...
	@Override
	public void renderItem(ItemRenderType type, @NotNull ItemStack itemstack, Object... data) {
	...
	private void renderItemInInventory(ItemStack itemstack, @NotNull EnumElement element, @NotNull Icon testtube, @NotNull Icon contents) {
	...
	private void renderItemInEquipped(ItemStack itemstack, @NotNull EnumElement element, Icon testtube, @NotNull Icon contents) {
	    /*
	...
	private void renderItemAsEntity(ItemStack itemstack, @NotNull EnumElement element, @NotNull Icon testtube, @NotNull Icon contents) {
		GL11.glPushMatrix();
		setColorForElement(element);
	...
	private void drawTextureIn3D(@NotNull Icon texture) {
		Tessellator tesselator = Tessellator.instance;
		float scale = 0.7F;
		GL11.glPushMatrix();
		GL11.glScalef(scale, scale, scale);
		ItemRenderer.renderItemIn2D(tesselator, texture.getMaxU(), texture.getMinV(), texture.getMinU(), texture.getMaxV(),
				texture.getIconWidth(), texture.getIconHeight(), .05F);
		GL11.glPopMatrix();
	}

	private void drawTexturedRectUV(float x, float y, float z, int w, int h, @NotNull Icon icon) {
		Tessellator tesselator = Tessellator.instance;
	...
	private void setColorForElement(@NotNull EnumElement element) {
