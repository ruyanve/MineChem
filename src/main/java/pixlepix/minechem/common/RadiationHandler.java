...
public class RadiationHandler {
	...
	public void update(@NotNull EntityPlayer player) {
	...
	@NotNull
	public List<DecayEvent> updateRadiationOnItems(@NotNull World world, @NotNull List<ItemStack> itemstacks) {
		return updateRadiationOnItems(world, null, null, itemstacks);
	}

	public int getTicksUntilDecay(@NotNull ItemStack itemstack, @NotNull World world) {
		RadiationInfo info = ItemElement.getRadiationInfo(itemstack, world);
		return (int) (info.radiationLife);
	}

	private void updateChemicalStorageContainer(@NotNull EntityPlayer player, Container openContainer) {
	...
	private void updateContainer(@NotNull EntityPlayer player, @NotNull Container container) {
		List<ItemStack> itemstacks = container.getInventory();
		updateRadiationOnItems(player.worldObj, player, container, itemstacks);
	}

	@NotNull
	private List<DecayEvent> updateRadiationOnItems(@NotNull World world, @NotNull EntityPlayer player, @Nullable Container container, @NotNull List<ItemStack> itemstacks) {
	...
	private void applyRadiationDamage(@NotNull EntityPlayer player, Container container, int damage) {
	...
	private void printRadiationDamageToChat(@NotNull EntityPlayer player, @NotNull DecayEvent decayEvent) {
	...
	private int updateRadiation(@NotNull World world, @NotNull ItemStack element) {
	...
	private int decayElement(@NotNull ItemStack element, @NotNull RadiationInfo radiationInfo, long currentTime, @NotNull World world) {
