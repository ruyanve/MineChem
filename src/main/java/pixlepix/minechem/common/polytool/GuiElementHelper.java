...
public class GuiElementHelper {
	...
	public void draw(@NotNull GuiPolytool gui, long ticks) {
		...
		/*}else{
			...
			int originX=88;
			int originY=50;
			int targetX=(int) (originX+(Math.sin(radians)*dist));
			int targetY=(int) (originY+(Math.cos(radians)*dist));
			
			int progressX=(int) (originX+((ticks/120)*(targetX-originX)));
			int progressY=(int) (originY+((ticks/120)*(targetY-originY)));
